...
##      (try_end),
  ...
  (1, [(neg|map_free),
       ...
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_weapon_mainy"),
         (troop_raise_skill, "trp_player", "skl_weapon_main", 1),
         (str_store_string, s2, "@ Your weapon main skill has increased by 1."),
       (else_try),
 ...
 # reserved for future use. For backward compatibility, we need to use these triggers instead of creating new ones.
  ...
  (24*7, # proficiency loss
   [(eq, "$proficiency_loss", 1),
    (try_for_range, ":proficiency", 0, 6),
	  ...
	  (try_begin),
	    (eq, ":proficiency", 0),
		(str_store_string, s10, "@mainy in one handed weapons"),
	  (else_try),
	    (eq, ":proficiency", 1),
		(str_store_string, s10, "@mainy in two handed weapons"),
	  (else_try),
	    (eq, ":proficiency", 2),
		(str_store_string, s10, "@mainy in polearms"),
	  (else_try),
	    (eq, ":proficiency", 3),
		(str_store_string, s10, "@mainy in archery"),
	  (else_try),
	    (eq, ":proficiency", 4),
		(str_store_string, s10, "@mainy in crossbows"),
	  (else_try),
	    (eq, ":proficiency", 5),
		(str_store_string, s10, "@mainy in throwing weapons"),
	  (try_end),
	  (lt, reg5, 0),
  ...
  (24,
   [(val_sub, "$morale_prisoner", 1),
	...
	(store_random_in_range, "$daily_random", 0, 10000), ## store a random number that can be used in various situations -- right now only used for random prices for ramun the subordinate trader
	]),
